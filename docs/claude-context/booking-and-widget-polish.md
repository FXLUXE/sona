...
**#2 ICP GATE — DONE (2026-06-29).** Shared `looksLikeChainName(name)` + `KNOWN_CHAINS` regex now live in `lib.ts` (base module; `prospects.ts` imports it — `prospects→lib` only, no circular). Finder (`prospects.ts findProspects`): skips candidate if OSM `brand`/`brand:wikidata`/`brand:wikipedia` tag present OR name matches KNOWN_CHAINS; new `skippedChain` count threaded through `FindResult` → `/api/outreach/find` JSON → dashboard note ("N national chains/franchises filtered out", `dashboard.ts` ~L914). Onboarding (`claimTenant`): **soft-warn** (Daniel's MCQ pick) — chains ALLOWED in, but on the new-tenant insert path `alertFounderChainSignup` console.warns + emails `cfg.adminEmail` via Resend (best-effort, never blocks). Verified fresh: Greggs/PureGym/Kwik Fit/Specsavers/Toni&Guy/Vets4Pets → CHAIN; Yazz Hair, Richard Ward, Bob's Barbers, indie cafes → keep. typecheck + `bun run check` (embedded-JS parse) both pass. Eval-neutral.

**STILL PENDING:** **250-point error sweep** across all 3 demo sites + widget + email + front/back end, as a marked-off task list. NOTE for sweep: impeccable hook flagged pre-existing layout-transition animations in `dashboard.ts` L199 + L249 (animating width/height/padding/margin → jank; use transform/opacity or grid-template-rows) — left untouched, fold into the sweep.
